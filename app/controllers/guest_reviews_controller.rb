class GuestReviewsController < ApplicationController
  ...
  def create
    # Step1: Check if the reservations exist (room_id, guest_id, guest_id)

    # Step2: Check if the current guest already reviewed thew guest in this reservation.
    ...
    if !@reservation.nil? && @reservation.room.user.id == guest_review_params[:host_id].to_i
      ...
      @guest_reviewed = GuestReview.where(
